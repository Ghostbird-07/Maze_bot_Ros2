...
from mazebot.utilities import ret_smallest_obj, ret_largest_obj, get_centroid, connect_objs
import mazebot.config as config
...
class bot_localizer():
    ...
    def extract_bg(self, frame):
        try:
            ...
            if hull is None:
                print("Error: Could not find maze boundaries")
                return False
    ...
    def get_car_loc(self, car_cnt, car_mask):
        try:
            ...
            print(f"Error in get_car_loc: {e}")
            self.loc_car = (0, 0)
            return False
    ...
    def localize_bot(self, curr_frame, frame_disp):
        try:
            ...
            if not self.is_bg_extracted:
                success = self.extract_bg(curr_frame.copy())
                if success:
                    self.is_bg_extracted = True
                else:
                    print("Background extraction failed")
                    return False
            ...
            if car_cnt is None:
                print("Warning: Car not detected in current frame")
                return False
            ...
            # Apply visualization to the display frame
            car_mask_idx = car_mask > 0
            if car_mask_idx.any():
                frame_disp[car_mask_idx] = frame_disp[car_mask_idx] + (0, 64, 0)
            
            car_circular_mask_idx = car_circular_mask > 0
            if car_circular_mask_idx.any():
                frame_disp[car_circular_mask_idx] = (0, 0, 255)
            ...
            print(f"Error in localize_bot: {e}")
            return False